...
//get distance
router.route('/khoangcach/:from/:to').get((req,res)=>{
    dboperations.getDistance(req.params.from,req.params.to).then(result => {
        res.json(result);
    })
});
...
router.route('/khachhang').post((req,res)=>{
    let item = {...req.body}
    dboperations.updKhachHang(item).then(result => {
        res.json(req.body);
    })
});

router.route('/khachnhan').put((req,res)=>{
    let item = {...req.body}
...
router.route('/phieuyeucau').put((req,res)=>{
    let item = {...req.body}
    dboperations.addPhieuYeuCau(item).then(result => {
        res.json(result[0]);
    })
});

router.route('/khachhang').put((req,res)=>{
    let item = {...req.body}
